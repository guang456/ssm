...
public class UserServiceImpl implements UserService {
	...
	public void add(TbUser user) {
		...
		user.setCreated(new Date());//会员的创建时间
		user.setUpdated(new Date());//修改时间
		user.setSourceType("1");//注册来源
		user.setPassword(DigestUtils.md5Hex(user.getPassword()));//实现密码加密
	...
	public void createSmsCode(final String phone) {
		...
		jmsTemplate.send(smsDestination, new MessageCreator() {
			...
			public Message createMessage(Session session) throws JMSException {
				...
				mapMessage.setString("template_code", "SMS_169898905");//模板编号
				mapMessage.setString("sign_name", "你是谁家的芙蓉");//签名
				Map m=new HashMap<>();
				m.put("number", code);
